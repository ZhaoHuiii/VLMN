...
class GradCAM:
    def __init__(self, model, target_layer):
        ...
        self.hook_forward = target_layer.register_forward_hook(self.save_activations)
        self.hook_backward = target_layer.register_backward_hook(self.save_gradients)
    ...
    def generate_cam(self, image_inputs, text_inputs=None, target_class=None):
        with torch.enable_grad():

            self.model.eval()
            image_inputs = {k: v.to(device_blip) for k, v in image_inputs.items()}
            
            vision_outputs = self.model.img_model(**image_inputs)
            cls_token = vision_outputs.last_hidden_state[:, 0]
            reduced_vision = F.relu(self.model.vision_fc(cls_token))
            
            dummy_text = torch.zeros_like(reduced_vision).to(device_cal)
            combined = torch.cat((reduced_vision, dummy_text), dim=1)
            logits = self.model.fc2(self.model.dropout(F.relu(self.model.fc1(combined))))
            ...
            cam = (activations * alpha).sum(dim=-1)
            
            patch_size = self.model.img_model.config.patch_size
            num_patches = int((self.model.img_model.config.image_size // patch_size) ** 2)
            cam = cam.reshape(cam.size(0), 
                            self.model.img_model.config.image_size // patch_size,
                            self.model.img_model.config.image_size // patch_size)
            
            cam = F.interpolate(cam.unsqueeze(1), 
                              size=image_inputs['pixel_values'].shape[2:], 
                              mode='bicubic',
...
device_blip = torch.device("cuda:2")
device_llama = torch.device("cuda:3")
device_cal = torch.device("cuda:2")
...
class MultiModalClassifier(torch.nn.Module):
    ...
    def forward(self, image_inputs, text_inputs):
        reduced_vision = self.img_encoder(image_inputs)
        reduced_text = self.text_encoder(text_inputs)
        
        contrastive_loss = self.contrastive_loss(reduced_vision, reduced_text)
        combined = torch.cat((reduced_vision, reduced_text), dim=1)
    ...
    def img_encoder(self, img_inputs):
        img_inputs = {k: v.to(device_blip) for k, v in img_inputs.items()}
        vision_outputs = self.img_model(**img_inputs)
        pooled_vision = torch.mean(vision_outputs.last_hidden_state, dim=1)
        return F.relu(self.vision_fc(pooled_vision)).to(device_cal)
    ...
    def text_encoder(self, text_inputs):
        ...
        pooled_text = torch.mean(text_outputs.hidden_states[-1], dim=1)
        return F.relu(self.text_fc(pooled_text)).to(device_cal)
...
def train_model(model, train_loader, val_loader, test_loader, resultPath, criterion, optimizer, scheduler, num_epochs=5, lambda_contrastive=0.1):
    ...
    resultPath = os.path.join(resultPath, timestamp)

    if not os.path.exists(resultPath):
        os.makedirs(resultPath)

    os.makedirs(os.path.join(resultPath, "cam_results"), exist_ok=True)
    os.makedirs(os.path.join(resultPath, "heatmap_results"), exist_ok=True)
    ...
    gradcam = GradCAM(model, model.img_model.encoder.layers[-8].layer_norm1)
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), os.path.join(resultPath, 'best_model.pth'))
    ...
    with torch.no_grad():
        for image_inputs, text_inputs, labels, patient_paths in tqdm(test_loader, desc="Testing"):
            ...
            with torch.set_grad_enabled(True):
                ...
                logits_for_cam, _ = model(image_inputs, text_inputs)
                logits_for_cam = logits_for_cam.to(device_cal)
            
                cam = gradcam.generate_cam(image_inputs, text_inputs)
            ...
            for idx, (path, prob, pred, label) in enumerate(zip(patient_paths, probabilities, preds, labels)):
                img = cv2.imread(path)
                img = cv2.resize(img, (224, 224))
                
                cam_resized = cv2.resize(cam, (img.shape[1], img.shape[0]))
                heatmap = cv2.applyColorMap(np.uint8(255 * cam_resized), cv2.COLORMAP_JET)
                superimposed = cv2.addWeighted(img, 0.7, heatmap, 0.3, 0)
                
                save_name = f"{os.path.basename(path).split('.')[0]}_cam.jpg"
                
                cv2.imwrite(os.path.join(resultPath, "heatmap_results", save_name), heatmap)
                cv2.imwrite(os.path.join(resultPath, "cam_results", save_name), superimposed)
    ...
    df.to_csv(os.path.join(resultPath, 'test_results.csv'), index=False)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(resultPath, f'training_results_{timestamp}.png'))
    plt.close()
    ...
    plt.savefig(os.path.join(resultPath, f'roc_curve_{timestamp}.png'))
    plt.close()
    ...
    plt.savefig(os.path.join(resultPath, f'pr_curve_{timestamp}.png'))
    plt.close()
