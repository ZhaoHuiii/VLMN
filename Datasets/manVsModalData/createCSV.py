...
def process_patient_folder1(patient_folder, type):

    data = []
    laryngoscope_folder = os.path.join(patient_folder, '喉镜图像')
...
def generate_csv(data_dir1, output_csv, type1):
    ...
    for patient_folder in os.listdir(data_dir1):
        ...
        if os.path.isdir(patient_path):
            ...
            patient_data = process_patient_folder1(patient_path, type1)
            all_data.extend(patient_data)

    df = pd.DataFrame(all_data)
    df.to_csv(output_csv, index=False)

data_dir1 = '/data/manVsModalData/WhitePatch'
output_csv = 'Datasets/manVsModalData/csv/whitePatch.csv'
type1 = 0
